      ...
      <div>
          ...
          <section class="">
              <section class="question-list lh-bj-list pr">
                  <ul class="pr10">
                      <li v-for="comment in commentData.items" :key="comment.id">
                          <aside class="noter-pic">
                              <img
                                  width="50"
                                  height="50"
          ...
          <div class="paging">
              ...
              <a
                ...
                href="#"
                title="前一页"
                @click.prevent="gotoPage(commentData.current - 1)"
                >&lt;</a
                  >
              <a
                ...
                :key="page"
                :class="{
                        current: commentData.current == page,
                        undisable: commentData.current == page,
                        }"
                :title="'第' + page + '页'"
                href="#"
              ...
              <a
                ...
                href="#"
                title="末页"
                @click.prevent="gotoPage(commentData.pages)"
                >末</a
                  >
              <div class="clear" />
          </div>
...
data() {
  ...
  methods: {
    ...
    //初始化课程评论
    initComment(){
      commentApi.getCommentInfo(this.page,this.limit,this.courseId).then(response=>{
    ...
    //分页切换
     gotoPage(page){
          commentApi.getCommentInfo(page, this.limit,this.courseId).then(response => {
              this.commentData = response.data.data
          })
      },
      //点击立即购买创建订单
